...
export const productVariants = mysqlTable("product_variants", {
  ...
  createdAt: timestamp("createdAt").defaultNow().notNull(),
  updatedAt: timestamp("updatedAt").defaultNow().onUpdateNow().notNull(),
}, (table) => ({ variantIndex: uniqueIndex("product_variants_product_color_idx").on(table.productId, table.colorCode) }));
...
export const inventoryUnits = mysqlTable("inventory_units", {
  ...
  status: mysqlEnum("status", ["available", "sold", "reserved", "damaged"]).default("available").notNull(),
  soldAt: timestamp("soldAt"),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({ productStatusIndex: index("inventory_units_product_status_idx").on(table.productId, table.status) }));
...
export const sales = mysqlTable("sales", {
  ...
  paymentMethod: mysqlEnum("paymentMethod", ["cash", "card", "instapay"]).notNull(),
  status: mysqlEnum("status", ["completed", "voided"]).default("completed").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
}, (table) => ({ createdAtIndex: index("sales_createdAt_idx").on(table.createdAt) }));
...
export const skuPrintJobs = mysqlTable("sku_print_jobs", {
  ...
  status: mysqlEnum("status", ["queued", "exported", "printed"]).default("queued").notNull(),
  createdAt: timestamp("createdAt").defaultNow().notNull(),
});
