...
export async function fetchPosStatus(): Promise<PosStatus> {
  ...
  try {
    const response = await fetch(
      `${secrets.websiteApiBaseUrl}/api/pos/status`,
      {
...
async function fetchProductsFromApi(options?: {
  ...
  const secrets = loadSecrets();
  if (!secretsConfigured(secrets))
    throw new Error("POS secrets are not configured");
  ...
  if (preferDelta && lastUpdatedAt) {
    ...
    const incoming = mapRemoteProducts(
  ...
  const response = await fetch(
    `${secrets.websiteApiBaseUrl}/api/pos/products`,
    { headers }
  );
...
export async function getProducts(options?: {
  ...
  if (!configured) {
    return {
      products: applyPendingToStock(readCachedProducts(), pending),
      pendingCount,
      configured: false,
    };
  }
  try {
    ...
    const shouldFetch = Boolean(
      options?.force || status.changed || !readCachedProducts().length
    );
    const remote = shouldFetch
      ? await fetchProductsFromApi({
          force: options?.force,
          preferDelta: options?.preferDelta ?? status.changed,
        })
      : readCachedProducts();
    return {
      products: applyPendingToStock(remote, pending),
      pendingCount,
      configured: true,
    };
  } catch (error) {
...
export async function syncPending(): Promise<{
  ...
  const secrets = loadSecrets();
  if (!secretsConfigured(secrets))
    return { synced: 0, remaining: countPendingSales() };
  ...
  for (const sale of pending) {
    try {
      const response = await fetch(
        `${secrets.websiteApiBaseUrl}/api/pos/sales`,
        {
          method: "POST",
          headers: {
            "Content-Type": "application/json",
            "x-pos-key": secrets.posApiKey,
          },
          body: JSON.stringify({
